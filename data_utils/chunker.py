...
class Chunker:
    ...
    def __init__(
        ...
        self.transcript: T.List[TranscriptChunk] = json_to_transcript_chunks(transcript_path)
    ...
    def process(self) -> T.List[T.Dict]:
        ...
        for chunk in chunks:
            ...
            chunk_audio: AudioSegment = self.audio[chunk.start * 1000: chunk.end * 1000]
            chunk_filepath = os.path.join(self.save_directory, chunk_filename)
            chunk_audio.export(chunk_filepath, format="wav")
            results_dict.append({"audio": f"{chunk_filepath}", "transcript": chunk.text})
